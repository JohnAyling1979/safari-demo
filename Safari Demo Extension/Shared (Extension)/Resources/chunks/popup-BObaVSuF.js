import{b as t}from"./browser-BnURzfs5.js";async function y(){console.log("safari-demo:popup: Popup script loaded");const o=document.getElementById("app"),[n]=await t.tabs.query({active:!0,currentWindow:!0}),d=n?{title:n.title??"No title",url:n.url??"No URL"}:{title:"N/A",url:"N/A"},[{count:g},{sessionId:r,loadedAt:m},h]=await Promise.all([t.runtime.sendMessage({type:"getCount"}),t.runtime.sendMessage({type:"getSessionInfo"}),t.runtime.sendMessage({type:"getSharedFile"}).catch(()=>({pdfUrl:null}))]);t.runtime.sendMessage({type:"broadcastSharedFileUpdated"}).catch(()=>{});const f=r!=null?`${String(r).slice(0,8)}…`:"—",i=h?.pdfUrl??null,b=i!=null?`<a class="value url" id="sharedPdfLink" href="${s(i)}" target="_blank" rel="noopener">${s(i)}</a>`:'<p class="value" id="sharedPdfLink">—</p>';o.innerHTML=`
    <div class="popup">
      <h1>Safari Demo</h1>
      <section>
        <h2>Shared PDF</h2>
