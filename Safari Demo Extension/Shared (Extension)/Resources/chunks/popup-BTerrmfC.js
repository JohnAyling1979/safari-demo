import{b as n}from"./browser-BnURzfs5.js";async function E(){console.log("safari-demo:popup: Popup script loaded");const d=document.getElementById("app"),[s]=await n.tabs.query({active:!0,currentWindow:!0}),i=s?{title:s.title??"No title",url:s.url??"No URL"}:{title:"N/A",url:"N/A"},g=s?.id&&s?.url&&(s.url.startsWith("http://")||s.url.startsWith("https://"))?n.runtime.sendMessage({type:"checkCurrentTabIsPdf",tabId:s.id}).catch(()=>({isPdf:!1})):Promise.resolve({isPdf:!1}),[{count:m},{sessionId:l,loadedAt:b},h,y]=await Promise.all([n.runtime.sendMessage({type:"getCount"}),n.runtime.sendMessage({type:"getSessionInfo"}),n.runtime.sendMessage({type:"getSharedFile"}).catch(()=>({pdfUrl:null})),g]),v=l!=null?`${String(l).slice(0,8)}…`:"—",o=h?.pdfUrl??null,I=o!=null?`<a class="value url" id="sharedPdfLink" href="${a(o)}" target="_blank" rel="noopener">${a(o)}</a>`:'<p class="value" id="sharedPdfLink">—</p>',c=y?.isPdf===!0;d.innerHTML=`
    <div class="popup">
      <h1>Safari Demo</h1>
      <section>
        <h2>Shared PDF</h2>
      ...
      <section>
        <h2>Storage Demo</h2>
        <p>Message count: <strong id="count">${m??0}</strong></p>
        <div class="button-row">
      ...
      <section>
        ...
        <p class="value debug" id="sessionId">${a(v)}</p>
        <p class="label">Loaded at:</p>
        <p class="value debug" id="loadedAt">${a(b??"—")}</p>
        <div class="button-row">
